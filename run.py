...
from enumerateallpaths import enumerateallpaths
...
def main(args):
    ...
    if not opts.stoichiometry and not opts.sbml and not opts.getsbmltargets:
    ...
    if opts.cyclicparallel or opts.pathwayreconstruct:
        ...
        if 'reactome' in opts.name or 'pid' in opts.name:
            node_dict = pkl.load(open('node_dict.pkl','rb'))
        else:
            node_dict = {}
        '''
        ...
        if opts.pathwayreconstruct:
            ...
            timeend = time.time()
            print(f'time taken: {timeend - timebegin}')
        #if opts.nodb:
...
#
    ...
    if opts.sbml and not opts.nonparametric:
        H,_,__,stoichiometries,reversible_reactions = make_hypergraph(ROOTDIR+'/parsed/{0}'.format(opts.name),stoichiometry=True,return_reversible=True)
        print(('target: {0}'.format(opts.target)))
        sources,targets,high_penalty_sources,H_name_dict = getSourcesTargets2(opts.name,H,'hypergraph',opts.source,opts.target)
        sources.update(high_penalty_sources)
    ...
    if opts.stoichiometry:
        stoi = None
        unit_stoichiometries = False
        if opts.unit_stoichiometries:
            unit_stoichiometries = True
        timebegin = time.time()
        if opts.sbml:
            H,_,__,stoichiometries,negative_regulators,reversible_reactions = make_hypergraph(ROOTDIR+'/parsed/{0}'.format(opts.name),stoichiometry=True,return_reversible=True, keep_negative_regulators=True, unit_stoichiometries=unit_stoichiometries)
        else:
            H,_,__,stoichiometries,negative_regulators = make_hypergraph(ROOTDIR+'/parsed/{0}'.format(opts.name),stoichiometry=True,keep_negative_regulators=True, unit_stoichiometries=unit_stoichiometries)
            reversible_reactions = None
        if opts.unit_stoichiometries:
            ___,_,__,stoi,_____ = make_hypergraph(ROOTDIR+'/parsed/{0}'.format(opts.name),stoichiometry=True,keep_negative_regulators=True)
        #print('numhyperedges',len(H.get_hyperedge_id_set()))
        #print(len(stoichiometries))
        #print(stoichiometries,negative_regulators)
        print(('target: {0}'.format(opts.target)))
        ...
        #print('len sources',len(sources))
        #source,target,stoichiometries = add_super_nodes(H,H_sources,H_targets,H_high_penalty_sources,opts.name,stoichiometries=stoichiometries)
        ilpname = '/tmp/reactome-%s-%s-stoichiometry.lp%s' % (opts.name,opts.type,opts.ilpchar)
        outprefix = '%s/results/reactome-%s-%s-stoichiometry%s' % (ROOTDIR,opts.name,opts.type,opts.ilpchar)
        #source_edges,H,stoichiometries,negative_regulators = find_source_edges(H,sources,add_edges=True,stoichiometries=stoichiometries,negative_regulators=negative_regulators)
        source_edges = find_source_edges(H,sources,add_edges=False,stoichiometries=stoichiometries,negative_regulators=negative_regulators)
        if opts.second_order_neg_reg:
            ...
            for i in range(1,7):
                ...
                print(maxinst)
                print('\n\n')
        elif opts.nonparametric:
            timebegin = time.time()
            nonparailpname = f'{ilpname}_nonparametric'
            try:
                timeacc = find_nonparametric_factory(nonparailpname, ilpname, H, stoichiometries, targets, source_edges, sources, solnum=opts.ilpchar, reversible=reversible_reactions, extra_stoichiometries=stoi)
                print('solved first MILP\n')
                print(f'time to solve acc nonparametric {timeacc - timebegin}')
            except:
                traceback.print_exc()
                print('not able to solve first ILP\n')
                timeacc = time.time()

            consilpname = ilpname + 'cons'
            consnonparailpname = ilpname + 'cons' + 'nonparametric'
            try:
                timecons = find_nonparametric_factory(consnonparailpname, consilpname, H, stoichiometries, targets, source_edges, sources, solnum=opts.ilpchar, positiveflux=False, reversible=reversible_reactions)
                print('solved conservation MILP\n')
                print(f'time to solve acc nonparametric {timecons - timeacc}')
            except:
                traceback.print_exc()
                print('not able to solve conservation ILP\n')
            ...
            #write_stoichiometry_ilp(ilpfile,H,stoichiometries,targets,source_edges,sources=sources,positiveflux=True,binary_vars=write_binary,nonparametric=opts.nonparametric)
            ilpfile.close()
            if opts.nonparametric:
                nonparailpname = f'{ilpname}_nonparametric'
                nonparailpfile = open(nonparailpname,'w')
                write_stoichiometry_ilp(nonparailpfile,H,stoichiometries,targets,source_edges,sources=sources,positiveflux=True,binary_vars=write_binary,nonparametric=opts.nonparametric)
                nonparailpfile.close()
            try:
                ilp_soln, active_edges = solve_stoichiometry_ilp(ilpname,H,sources,binary_vars=write_binary,solnum=3)
                if opts.nonparametric:
                    #solve_stoichiometry_ilp_nonparametric(ilpname,H,sources,binary_vars=write_binary,solnum=opts.ilpchar)
                    solve_stoichiometry_ilp_nonparametric(nonparailpname,H,sources,binary_vars=write_binary,solnum=opts.ilpchar,doubling=True,starting_k=len(active_edges))
                print('solved first MILP\n')
                #print('neg\n')
            except:
                traceback.print_exc()
                ...
                if opts.nonparametric:
                    #solve_stoichiometry_ilp_nonparametric(ilpname,H,sources,binary_vars=write_binary,solnum=opts.ilpchar)
                    solve_stoichiometry_ilp_nonparametric(nonparailpname,H,sources,binary_vars=write_binary,solnum=opts.ilpchar,doubling=True)
            ilpfile = open(ilpname + 'cons','w')
            write_binary = True
            ...
            #write_stoichiometry_ilp(ilpfile,H,stoichiometries,targets,source_edges,sources=sources,positiveflux=False,binary_vars=write_binary,nonparametric=opts.nonparametric)
            ilpfile.close()
            if opts.nonparametric:
                nonparailpname = ilpname + 'cons' + 'nonparametric'
                nonparailpfile = open(nonparailpname,'w')
                write_binary = True
                write_stoichiometry_ilp(nonparailpfile,H,stoichiometries,targets,source_edges,sources=sources,positiveflux=False,binary_vars=write_binary,nonparametric=opts.nonparametric)
                nonparailpfile.close()
            try:
                ilp_soln, active_edges = solve_stoichiometry_ilp(ilpname,H,sources,binary_vars=write_binary,solnum=3)
                if opts.nonparametric:
    ...
    if opts.getsbmltargets:
        H,_,__,stoichiometries,negative_regulators,reversible_reactions = make_hypergraph(ROOTDIR+'/parsed/{0}'.format(opts.name),stoichiometry=True,return_reversible=True, keep_negative_regulators=True)
        #node_dict = printremappededges(H)
        node_dict=None
        test_source_sink_pairs_for_cycles(H,node_dict,opts,firsttimetargets=True, organism=opts.ilpchar)
    ...
    if opts.heuristic:
        ...
        #print(bestp)
        #print(bestweight)
        #find_alt_paths(P,H2,source,target,node_dict=node_dict)
...
def test_source_sink_pairs_for_cycles(H,node_dict,opts,targetlist=None,firsttimetargets=True, organism=None):
    if organism != None:
        fileappendage = organism
    else:
        fileappendage = 'ecoli'
    if targetlist == None:
...
def make_hypergraph(file_prefix,delim=';',sep='\t',keep_singleton_nodes=False,stoichiometry=False,keep_negative_regulators=False,return_reversible=False,target=None,select_edges=[], unit_stoichiometries=False):
    hypernodes = {}
...
def parseOptions(args):
    ...
    parser.add_option('','--tail_size',action='store_true',help='add cheat hyperedges')
    parser.add_option('','--getsbmltargets',action='store_true',help='add cheat hyperedges')
    parser.add_option('','--nonparametric',action='store_true',help='find factories without a value for epsilon')
    parser.add_option('','--unit_stoichiometries',action='store_true',help='set all stoichiometries to 1')
