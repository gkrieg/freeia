...
from stoichiometryilp import write_vertex_flux_constraints,write_target_constraints,write_edge_flux_constraints
...
def write_shortest_hyperpath_factory_ilp(H,ilpfilename,target,source,realvaluedcuts=False):
    '''
    This used to have the factory constraints within it, either with real-valued cuts or with integer flux constraints
    This has been deprecated for the function below write_shortest_hyperpath_hybrid_ilp
    '''
    ...
    ## set stoichiometry ratios
    stoichiometries = get_fixed_stoichiometries(H)
    ## write constraints
    if realvaluedcuts == True:
        write_vertex_flux_constraints(ilpfile,H,stoichiometries,[target],sources=[source],positiveflux=True)
        #need to constrain edge fluxes to be between 0 and 1 in this case
        write_edge_flux_constraints(ilpfile,H,[target])
        for e in H.hyperedge_id_iterator():
            ilpfile.write('{} - {} >= 0\n'.format(a(e),fedge(e)))
    else:
        write_vertex_flux_constraints(ilpfile,H,stoichiometries,[target],sources=[source],positiveflux=True,aedge=True)
    epsilon = 1.0
    if realvaluedcuts == True:
        write_target_constraints(ilpfile,H,[target],stoichiometries,epsilon)
    else:
        write_target_constraints(ilpfile,H,[target],stoichiometries,epsilon,aedge=True)

    #Binary edge constraints
        ...
        if not p.is_alive() and headcutsadded == False:
            ...
            for ii in range(len(return_dict[1])):
                if return_dict[1][ii] != b'\x00':
                    ...
                    break
            newheadcuts = unserialize_vertex_headcuts(serialvertexheadcuts)
            if len(newheadcuts) > 0:
                headcuts = newheadcuts
            #print(edgeheadcuts)
